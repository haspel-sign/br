...
class Validator
{
    ...
    public function make($field, $label, $rules)
    {
        $this->_rule_data[] = [
            'field' => $field,
            'label' => $label,
                ...
                foreach ($data['rules'] as $_rules) {
                    ...
                    if ($_run === false) {
                        ...
                        if ($rule == 'required') {
                            ...
                            $this->_errors[$data['field']][0] = $this->_msg('required', $label, $arg);

                            break;
                        }
                        $this->_errors[$data['field']][] = $this->_msg($rule, $label, $arg);
                    }
     ...
     * @return mixed
     */
    private function _msg($rule, $label = null, $arg = null)
    {
     ...
     * @return mixed
     */
    public function _get_argument($a)
    {
    ...
    {
        throw new \Exception('Няма такава валидация ' . $a, 500);
    }
